...
**You are reading the companion to Paper 3** (this repo: https://github.com/Ethan-Brooke/APF-Paper-3-Ledgers-Entropy-Time-Cost).
...
When a user's question is about a result outside Paper 3, point them to the relevant row above. Do not attempt to re-derive cross-paper results from the bundled subset — the canonical state is the codebase ([Zenodo concept DOI](https://doi.org/10.5281/zenodo.18529115)), not this slice.

---

## 1. What this repository is

This is the **paper-companion release repo for Paper 3 of the Admissibility Physics Framework (APF) series**. Specifically: Ledgers: Admissible Dynamics, Entropy, and the Arrow of Time.

The repo bundles:

- The manuscript and Technical Supplement (`.tex` + `.pdf`)
- A vendored subset of the APF codebase containing only the `check_*` functions that Paper 3 directly references (`apf/core.py`, `apf/apf_utils.py`, `apf/bank.py`)
...
The framework-wide canonical codebase (342 bank-registered theorems, 19 modules, 355 `verify_all` checks) is **not** bundled here. Only the Paper 3 subset is runnable locally. See `ai_context/THEOREMS.md` for the full bank listing with bundled-in-this-repo flags.
...
If you're loading this repo cold and want to understand Paper 3 well enough to reason about it:
...
7. **`Paper_3_Ledgers_Entropy_Time_Cost_v3.2.tex`** or `.pdf` — the manuscript. Read §1 (intro) and the section you care about. Don't try to read the whole paper first.
