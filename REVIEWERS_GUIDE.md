# Reviewers' Guide — APF Paper 3: Ledgers: Admissible Dynamics, Entropy, and the Arrow of Time

This guide is the physics-first walkthrough for peer reviewers of Ledgers: Admissible Dynamics, Entropy, and the Arrow of Time. It addresses the structural assumptions, anticipated objections, and falsification surfaces of the paper's argument. The intended audience is a peer reviewer with a background in physics or mathematics.

The guide complements three other verification routes:
- The [executable codebase](apf/) (run `python run_checks.py` for the full 13-theorem verification);
- The [Colab notebook](APF_Reviewer_Walkthrough.ipynb) (zero-install, theorem-by-theorem cells with prose + code);
- The [interactive derivation DAG](https://ethan-brooke.github.io/Ledgers-Entropy-Time-Cost/) (hover-for-details, click-for-dependencies, animated verification).

---

## The derivation in 13 steps

The full chain from the foundational commitments to this paper's results:

**Step 1 — T_CPTP** (`check_T_CPTP`)

  T_CPTP: CPTP Maps from Admissibility-Preserving Evolution.


**Step 2 — T_kappa** (`check_T_kappa`)

  T_kappa: Directed Enforcement Multiplier.


**Step 3 — T_entropy** (`check_T_entropy`)

  T_entropy: Von Neumann Entropy as Committed Capacity.


**Step 4 — T_zeroth_law** (`check_T_zeroth_law`)

  T_zeroth_law: Zeroth Law of Thermodynamics [P].


**Step 5 — T_first_law** (`check_T_first_law`)

  T_first_law: First Law of Thermodynamics [P].


**Step 6 — T_second_law** (`check_T_second_law`)

  T_second_law: Second Law of Thermodynamics [P].


**Step 7 — T_deSitter_entropy** (`check_T_deSitter_entropy`)

  T_deSitter_entropy: de Sitter Entropy from Capacity Microstate Counting [P].


**Step 8 — L_irr** (`check_L_irr`)

  L_irr: Irreversibility from Admissibility Physics.


**Step 9 — L_irr_uniform** (`check_L_irr_uniform`)

  L_irr_uniform: Sector-Uniform Irreversibility.


**Step 10 — T_CPT** (`check_T_CPT`)

  T_CPT: CPT Invariance [P].


**Step 11 — L_Fisher_measure** (`check_L_Fisher_measure`)

  L_Fisher_measure: APF Capacity Counting Derives S = (d_eff/2) ln det G [P].


**Step 12 — L_crossing_entropy** (`check_L_crossing_entropy`)

  L_crossing_entropy: 1/α_cross = S_dS / 6 [P].


**Step 13 — L_Fisher_entropy_budget** (`check_L_Fisher_entropy_budget`)

  L_Fisher_entropy_budget: Generation Mixing Uses 17.1% of de Sitter Entropy [P].



Each step is a single theorem. The dependencies form a directed acyclic graph (DAG) with no cycles; the [interactive DAG](https://ethan-brooke.github.io/Ledgers-Entropy-Time-Cost/) shows multiple derivation paths to the same result, which is structural redundancy: many results are over-determined and would survive even if individual derivation steps were weakened.
...
**Paper 3 additionally relies on three hypotheses, flagged at point of use:**

- **H1 — Markovian-composition closure.** Composition of admissibility-preserving CPTP maps is admissibility-preserving. Used in $T_{\rm seq}$ (temporal Tsirelson).
- **H2 — Hamiltonian-constraint redundancy.** At a causal horizon, the only relevant gauge redundancy in the ACC-to-entropy conversion is time reparameterization. Used in the $1/2 \times 1/2$ horizon-entropy proof.
- **H3 — Constraint-functional monotonicity.** Along admissible cosmological trajectories, $\Phi_c = \ln(d_{\rm field}/d_{\rm admissible})$ is monotone non-increasing. Used in the macroscopic-arrow argument.

None of H1–H3 is a PLEC component; each is paper-3-local. Failures isolate the corresponding section without dissolving the framework.
...
To prevent scope inflation, the following are flagged explicitly as *outside* the scope of Ledgers Entropy Time Cost:
...
- **`apf/core.py`** — the 13 theorem check functions for this paper. Each function constructs a mathematical witness, verifies the theorem's claim, and returns a structured result with name, dependencies, status, and key result.
...
This repository is the executable mathematical appendix to APF Paper 3. The canonical archival deposit is at [https://doi.org/10.5281/zenodo.18604844](https://doi.org/10.5281/zenodo.18604844) (DOI: 10.5281/zenodo.18604844).
