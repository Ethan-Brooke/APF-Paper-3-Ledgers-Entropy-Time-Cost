...
AI agents have a recurring failure mode on this project: responding to APF requests with fluent-but-wrong output because the relevant context was never loaded. This file is the minimum context load to reason about Paper 3 of the APF series honestly.
...
**Canonical state (one source of truth).** Codebase **v6.9**, frozen 2026-04-18. Full engine: **355** verify_all checks, **342** bank-registered theorems, **48** quantitative predictions. This repo bundles 13 of those checks for Paper 3's dependency subset. If you see a different count anywhere, this line is canonical.
...
| 3 | Ledgers **(this repo)** | [10.5281/zenodo.18439363](https://doi.org/10.5281/zenodo.18439363) | [`APF-Paper-3-Ledgers-Entropy-Time-Cost`](https://github.com/Ethan-Brooke/APF-Paper-3-Ledgers-Entropy-Time-Cost) | public |
...
You are reading the companion to **Paper 3** (this repo: https://github.com/Ethan-Brooke/APF-Paper-3-Ledgers-Entropy-Time-Cost). The canonical computational engine lives at codebase **v6.9** ([Zenodo](https://doi.org/10.5281/zenodo.18529115)). Each paper has its own companion repo following the same layout.

**When in doubt about scope:** if the question is about a result outside Paper 3, point the user (or yourself) to the relevant paper above. Do not try to derive it from the bundled subset; the canonical state is the codebase, not this slice.
...
Expected output: `Paper 3 (Ledgers: Admissible Dynamics, Entropy, and the Arrow of Time): N passed, 0 failed, N total — verified in under 1s`.
...
- What this repo is (Paper 3 + vendored `apf/` subset)
...
Not mandatory to read end-to-end, but make sure you know it exists and how to query it. The full bank has 342 entries; this paper bundles **13 of 342** for local execution. Each entry has name, module, epistemic tag, dependencies, and one-line summary.
...
- **`Paper_3_Ledgers_Entropy_Time_Cost_v3.2.pdf`** — the manuscript. Read §1 (Introduction) and whichever section is relevant to your task. Don't try to read the whole paper before starting work.
- **`Paper_3_Ledgers_Entropy_Time_Cost_Supplement_*.pdf`** (if present) — the Technical Supplement. Canonical proofs. Dense; dip in only where you need depth.
